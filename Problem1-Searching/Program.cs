...
class SearchComparison
{
    static void Main()
    {
        ...
        foreach (int size in datasetSizes)
        {
            ...
            double linearSortingTime = MeasureExecutionTime(LinearSearch, data, target);
            double binarySortingTime = MeasureExecutionTime(BinarySearch, data, target);

            Console.WriteLine($"{size,11} | {linearSortingTime,19:F6} | {binarySortingTime,18:F6}");
        }
    }
